...
function ix.weight.CanCarry(weight, carry, character)
    local maxWeight = ix.config.Get("maxWeight", 30)
    local maxOverWeight = ix.config.Get("maxOverWeight", 5)
    local strengthModifier = character:GetAttribute("strength", 0) * StrhMulti
    local max = maxWeight + maxOverWeight + strengthModifier
...
local maxWeight = ix.config.Get("maxWeight", 30)
local maxOverWeight = ix.config.Get("maxOverWeight", 5)
local totalMaxWeight = maxWeight + maxOverWeight + strengthModifier
...
bar.Paint = function(self)
    ...
    surface.DrawRect(4, 4, math.min(((w - 8) / totalMaxWeight) * carry, w - 8), 16)
end
